...
// Contributor(s): Edwin Betancourt EdwinBetanc0rt@outlook.com www.erpya.com
...
const getters = {
  ...
  /**
   * Show all available fields not mandatory to show, used in components PanelDefinition/FilterFields.vue
  ...
  getFieldsListNotMandatory: (state, getters) => ({
    ...
    return getters.getFieldsListFromPanel(containerUuid)
      .filter(fieldItem => {
        const isButton = fieldItem.componentPath === 'FieldButton'
        const isMandatory = fieldItem.isMandatory || fieldItem.isMandatoryFromLogic
        if (!isMandatory) {
          if (isEvaluateShowed) {
            return fieldIsDisplayed(fieldItem) && !isButton
          }
          return !isMandatory && !isButton
        }
      })
  },
